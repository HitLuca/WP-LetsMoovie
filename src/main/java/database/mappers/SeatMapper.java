...
public interface SeatMapper {
    ...
    @Select("SELECT id_seat FROM seats WHERE room_number=#{room_number} AND row=#{row} AND \"column\"=#{column}")
    ...
    @Select("SELECT status FROM seat_reservations WHERE id_seat=#{id_seat} AND id_show=#{id_show}")
    String getReservationStatus(@Param("id_seat") int id_seat, @Param("id_show") int id_show);

    //TODO:Test
    @Select("SELECT price FROM payments NATURAL JOIN prices WHERE id_seat=#{id_seat} AND id_show=#{id_show}")
    float getSeatPayment(@Param("id_seat") int id_seat, @Param("id_show") int id_show);

    //TODO:Test
    @Select("SELECT sr.id_seat, COUNT(*) FROM seat_reservations sr JOIN seats s ON sr.id_seat=s.id_seat WHERE room_number=#{room_number} AND sr.status='reserved' GROUP BY sr.id_seat ORDER BY sr.id_seat DESC")
    List<Seat> getRankedSeatReservations(int room_number);
}
